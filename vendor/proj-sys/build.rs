...
fn build_from_source() -> Result<std::path::PathBuf, Box<dyn std::error::Error>> {
    ...
    if let Some((sqlite_include, sqlite_library)) = bundled_sqlite_paths() {
        eprintln!("using bundled sqlite for proj build: {}", sqlite_library.display());
        config.define("SQLITE3_INCLUDE_DIR", sqlite_include);
        config.define("SQLITE3_LIBRARY", sqlite_library);
    }
...
fn bundled_sqlite_paths() -> Option<(PathBuf, PathBuf)> {
    if let (Ok(sqlite_include), Ok(sqlite_lib_dir)) = (
        env::var("DEP_SQLITE3_INCLUDE"),
        env::var("DEP_SQLITE3_LIB_DIR"),
    ) {
        let sqlite_library = preferred_sqlite_library(PathBuf::from(sqlite_lib_dir))?;
        return Some((PathBuf::from(sqlite_include), sqlite_library));
    }
    ...
    for entry in entries.flatten() {
        ...
        for line in output.lines() {
            if let Some(value) = line.strip_prefix("cargo:include=") {
                include_dir = Some(PathBuf::from(value.trim()));
            } else if let Some(value) = line.strip_prefix("cargo:lib_dir=") {
        ...
        let sqlite_library = preferred_sqlite_library(lib_dir)?;
        return Some((include_dir, sqlite_library));
    }

    None
}

fn preferred_sqlite_library(lib_dir: PathBuf) -> Option<PathBuf> {
    let sqlite_lib = lib_dir.join("sqlite3.lib");
    if sqlite_lib.exists() {
        return Some(sqlite_lib);
    }

    let libsqlite = lib_dir.join("libsqlite3.a");
    if libsqlite.exists() {
        return Some(libsqlite);
    }
