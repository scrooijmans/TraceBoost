...
fn discover_default_dataset_paths(
    ...
    let candidates = vec![
        root.join("test-data").join("small.sgy"),
        root.join("test-data").join("f3.sgy"),
        root.join("test-data").join("long.sgy"),
        root.join("segyio")
            .join("python")
            .join("tutorials")
            .join("viking_small.segy"),
        root.join("test-data").join("small-lsb.sgy"),
    ];
...
fn find_monorepo_root() -> Result<PathBuf, SeisRefineError> {
    ...
    Err(SeisRefineError::Message(
        "unable to locate monorepo root from CARGO_MANIFEST_DIR".to_string(),
    ))
}
